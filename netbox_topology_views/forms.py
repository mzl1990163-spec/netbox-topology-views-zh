...
class DeviceFilterForm(
    ...
    straight_cables = forms.BooleanField(
        label=_('直线线缆'), required=False, initial=False,
        widget=forms.Select(
            choices=BOOLEAN_WITH_BLANK_CHOICES
        )
    )
    draw_termination_labels = forms.BooleanField(
        label=_('端子标签'), required=False, initial=False,
        widget=forms.Select(
            choices=BOOLEAN_WITH_BLANK_CHOICES
        )
    )
    draw_cable_labels = forms.BooleanField(
        label=_('线缆标签'), required=False, initial=False,
        widget=forms.Select(
            choices=BOOLEAN_WITH_BLANK_CHOICES
        )
    )
    ...
    show_circuit = forms.NullBooleanField(
        label=_('显示电路端子'), required=False, initial=False,
    ...
    show_power = forms.NullBooleanField(
        label=_('显示电源馈线'), required=False, initial=False,
        help_text=_('将显示所有电源馈线。电源馈线无法筛选。'),
        widget=forms.Select(
            choices=BOOLEAN_WITH_BLANK_CHOICES
        )
    )
...
class PowerPanelCoordinatesForm(NetBoxModelForm):
    fieldsets = (
        FieldSet('group', 'device', 'x', 'y', name=_('配电盘')),
    )
...
class CoordinatesForm(NetBoxModelForm):
    fieldsets = (
        FieldSet('group', 'device', 'x', 'y', name=_('坐标')),
    )
