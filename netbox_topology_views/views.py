...
def create_edge(
    ...
    if circuit is not None:
        edge["dashes"] = True
        title = f"电路提供商: {circuit['provider_name']}<br>端子"
...
class TopologyImagesView(PermissionRequiredMixin, View):
    ...
    def get(self, request: HttpRequest):
        # 扫描图标分组:子目录=分组,根目录=内置组
        groups = {}
        builtin = [
            {"url": image_static_url(f), "name": f.name}
            for f in CONF_IMAGE_DIR.iterdir()
            if f.is_file() and f.suffix.lower().lstrip(".") in IMAGE_FILETYPES
        ]
        if builtin:
            groups["内置"] = builtin
        for d in sorted(p for p in CONF_IMAGE_DIR.iterdir() if p.is_dir()):
            icons = [
                {"url": image_static_url(f), "name": f.name}
                for f in d.iterdir()
                if f.is_file() and f.suffix.lower().lstrip(".") in IMAGE_FILETYPES
            ]
            # 即使是空分组(刚创建、尚未上传图标)也要列出,否则刷新后下拉里看不到新建的分组
            groups[d.name] = icons
        ...
        for r in roles.values():
            img = r.get("image") or ""
            r["group"] = next(
                (g for g, prefix in group_prefixes.items() if prefix in img),
                "内置",
            )

        roles_list = sorted(list(roles.values()), key=lambda r: r["name"])
        return render(
            request,
            "netbox_topology_views/images.html",
            {
                ...
                "roles_json": json.dumps(
                    {str(r["id"]): r.get("image") or "" for r in roles_list}
                ),
                "basepath": settings.BASE_PATH,
            },
        )
    ...
    def post(self, request: HttpRequest):
        """网页上传自定义图标到图标目录(持久卷),支持多文件 + 分组(选组/新建组)。"""
        uploads = request.FILES.getlist("image")
        if not uploads:
            messages.error(request, "请选择要上传的图标文件")
            return HttpResponseRedirect(request.path)

        # 目标分组:内置=根目录;否则为子目录(自动创建 = 新建分组)
        group = (request.POST.get("group") or "").strip().strip("/")
        if group and group != "内置":
            # 安全校验组名(仅字母数字-_)
            if not re.fullmatch(r"[A-Za-z0-9_-]+", group):
                messages.error(request, f"分组名 {group!r} 含非法字符,仅允许字母/数字/-/_")
                return HttpResponseRedirect(request.path)
            target_dir = CONF_IMAGE_DIR / group
            try:
                target_dir.mkdir(parents=True, exist_ok=True)
            except OSError as e:
                messages.error(request, f"创建分组失败: {e}")
                return HttpResponseRedirect(request.path)
        else:
            target_dir = CONF_IMAGE_DIR
        ...
        for upload in uploads:
            ...
            if ext not in IMAGE_FILETYPES:
                messages.warning(
                    request,
                    f"{upload.name}: 不支持的格式 {ext or '(无扩展名)'},已跳过(支持 png / svg / jpg / gif / webp / bmp 等)",
                )
                continue
            if upload.size > 2 * 1024 * 1024:
                messages.warning(request, f"{upload.name}: 文件过大(限 2MB),已跳过")
                continue
            # 防路径穿越:只取文件名
            ...
            try:
                ...
                messages.error(request, f"{upload.name}: 写入失败 {e}")

        if ok_count:
            msg = f"成功上传 {ok_count} 个图标"
            if group and group != "内置":
                msg += f"到分组 [{group}]"
...
class TopologyIndividualOptionsView(PermissionRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "选项已成功保存")
        else:
...
class SavedFiltersView(PermissionRequiredMixin, View):
    """
    我的拓扑:列出与设备相关的已保存过滤器(Saved Filter),
    点击跳转到拓扑视图并加载对应 filter_id。
    """
    permission_required = ("dcim.view_site", "dcim.view_device")
    ...
    def get(self, request: HttpRequest):
        ...
        saved_filters = (
            SavedFilter.objects.filter(object_types=ct, enabled=True)
            .order_by(Lower("name"))
        )
        return render(
            request,
            "netbox_topology_views/saved_filters.html",
            {
...
class CreateIconGroupView(PermissionRequiredMixin, View):
    """创建图标分组(立即建目录,独立于上传)。"""
    permission_required = ("dcim.view_site", "dcim.change_devicerole")

    def post(self, request: HttpRequest):
        from django.http import JsonResponse
        name = (request.POST.get("group") or "").strip().strip("/")
        if not name or name == "内置":
            return JsonResponse({"error": "分组名无效"}, status=400)
        if not re.fullmatch(r"[A-Za-z0-9_-]+", name):
            return JsonResponse({"error": "分组名仅允许字母/数字/-_"}, status=400)
        target = CONF_IMAGE_DIR / name
        try:
            target.mkdir(parents=True, exist_ok=True)
        except OSError as e:
            return JsonResponse({"error": f"创建失败: {e}"}, status=500)
        return JsonResponse({"ok": True, "group": name})
