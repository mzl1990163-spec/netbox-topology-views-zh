...
def get_model_role(model: Type[Model]) -> Role:
    # Built-in NetBox models: prefer localized display names
    MODEL_NAME_ZH = {
        'PowerPanel': '配电盘',
        'PowerFeed': '电源馈线',
        'Circuit': '电路',
    }
    zh = MODEL_NAME_ZH.get(model.__name__)
    return Role(
        slug=get_model_slug(model),
        name=zh if zh else re_camel_case.sub(r" \1", model.__name__),
    )
...
def get_query_settings(request):
    ...
    if save_coords == True and settings.PLUGINS_CONFIG["netbox_topology_views"]["allow_coordinates_saving"] == False:
        save_coords = False
        messages.warning(request, "坐标保存未获允许,设置已被覆盖")
